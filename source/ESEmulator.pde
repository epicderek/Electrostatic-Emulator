...
Float computeAcceleration(int first, int second)
{
...
boolean isIn(int index, float xPos, float yPos)
{
...
void populate(float x, float y, float chargeNet, float massNet)
{
  ...
  vel[0] = (Math.random()>0.5?1:-1)*(int)(Math.random()*velocityBound);
  vel[1] = (Math.random()>0.5?1:-1)*(int)(int)(Math.random()*velocityBound); 
...
{
  charges.add((Math.random()<0.5?1:-1)*3.00);
